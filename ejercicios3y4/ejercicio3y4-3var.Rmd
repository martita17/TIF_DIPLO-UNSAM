...
Prompt 2 (modo few shot prompting, se le da un ejemplo de cada categoria)
```{r}
unique(bmodelo_filt$orientacion)
# Obtener valores únicos de la columna 'orientacion'
unique(bmodelo_filt$orientacion)

# Filtrar por orientación conservadora
conservadores <- bmodelo_filt %>% 
  filter(orientacion == "+ conservador")
ej_conservador <- conservadores$orientacion[2]

# Filtrar por orientación progresista
progresistas <- bmodelo_filt %>% 
  filter(orientacion == "+ progresista")
ej_progresista <- progresistas$orientacion[2]

# Filtrar por orientación neutra
neutros <- bmodelo_filt %>% 
  filter(orientacion == "neutro")
ej_neutro <- neutros$orientacion[2]  # Corregí el nombre de la variable

# Construir el prompt correctamente
prompt2 <- paste(
  ...
  "",
  "No justifiques tu respuesta, ni des información adicional, sólo contesta con una de estas tres categorías: conservador, progresista y neutro.",
  "",
  "Este es el texto:"
)
...
for (i in 1:nrow(base_gem)){
                ...
                rta <- gemini(paste0(prompt1, base_gem$texto[i]))
                base_gem$pred[i]=rta
                Sys.sleep(4.5)
                }
